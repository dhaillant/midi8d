 ...
 *   Role                                Output   Pin     Type
 *   ----------------------------------------------------------------------
 *   Drum trig (MIDI Ch #10, note #36)   1        D2      Digital (On/Off)
 *   Drum trig (MIDI Ch #10, note #37)   2        D4      Digital (On/Off)
 *   Drum trig (MIDI Ch #10, note #38)   3        D7      Digital (On/Off)
 *   Drum trig (MIDI Ch #10, note #39)   4        D8      Digital (On/Off)
 * 
...
#define NBR_OUTPUTS 9
byte gate_pins[NBR_OUTPUTS] = {
  2, 4, 7, 8, 9, 3, 5, 6, 10
...
// Drum specific variables and declarations

// lowest note we want to respond to
#define DRUM_LOWEST_MIDI_NOTE 36
// 36 (decimal value for MIDI note number) corresponds to C2

// Number of outputs for drums
#define NBR_DRUM_OUTS 4

// then, highest note will be 
#define DRUM_HIGHEST_MIDI_NOTE DRUM_LOWEST_MIDI_NOTE + NBR_DRUM_OUTS - 1

// state of each drum gate output
bool drum_gate_states[NBR_DRUM_OUTS];

byte drum_gate_pins[NBR_DRUM_OUTS] = {
  2, 4, 7, 8
};    // array of output pin numbers (Arduino #)
...
//  MIDI.setHandleStop(handleStop);
  ...
  mono_allocator[0].Init();   // regular voice (with Pitch, velocity, etc)


  // drum-gate states initialization
  for (uint8_t i = 0; i < NBR_DRUM_OUTS; i++)
  {
    drum_gate_states[i] = LOW;
  }
...
void handleNoteOn(byte channel, byte pitch, byte velocity)
{
  ...
  else
  {
    if (channel == 10)
    {
      ...
      if ((pitch >= DRUM_LOWEST_MIDI_NOTE) && (pitch <= DRUM_HIGHEST_MIDI_NOTE))
      {
        drum_gate_states[pitch - DRUM_LOWEST_MIDI_NOTE] = HIGH;
      }
    }
    else
    {
...
void handleNoteOff(byte channel, byte pitch, byte velocity)
{
  ...
  if (channel == 10)
  {
    ...
    if ((pitch >= DRUM_LOWEST_MIDI_NOTE) && (pitch <= DRUM_HIGHEST_MIDI_NOTE))
    {
      drum_gate_states[pitch - DRUM_LOWEST_MIDI_NOTE] = LOW;
    }
  }
  else
  {
...
void render_gates(void)
{
  ...
  // for drums only (channel 10):
  for (uint8_t i = 0; i < NBR_DRUM_OUTS; i++)
  {
    digitalWrite(drum_gate_pins[i], drum_gate_states[i]);
  }
}
